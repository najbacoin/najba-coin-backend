...
// @Deprecated use ChunkActionV2
export interface ChunkActionInput<T, R> {
  action: (x: T[]) => Promise<R>;
  items: T[];
  sleepMs: number | null;
  chunkSize: number;
  operationName?: string;
}
export interface ChunkActionV2Input<T, R> {
  action: (x: T[]) => Promise<R>;
  items: T[];
  sleepMs: number | null;
  chunkSize: number;
  operationName?: string;
  errorHandler: (chunk: T[], error: any) => R;
}
...
export function promiseSequence<T>(promises: Array<() => Promise<T>>, delay: number = 0) {
...
export function removeNulls(array: Array<string | null>): string[] {
  return array.map(a => (a === null ? "" : a)).filter(x => x !== "");
}
